...
The system SHALL show wholesale users only active products enabled for wholesale.

#### Scenario: Wholesaler opens catalog
- **WHEN** a wholesale user views the catalog
- **THEN** the system shows only active products where `habilitadoMayorista` is true
...
- **AND** shows a success message and the pending order in history
...
- **AND** allows filtering by status in a future enhancement
